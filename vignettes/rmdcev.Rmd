...
\noindent
where $w_i$ is a vector of individual characteristics and $\delta_s$ is a vector of coefficients to be estimated that determine how these individual characteristics affect the membership of individual $i$ in segment $s$. For identification, the $\delta_1$ coefficients for the first segment are set to zero.
...
data_rec
```
The data includes information on three individual characteristics (university, ageindex, urban)\footnote on ageindex .more details are provided in Lloyd-Smith (2019). The names for each activiry are provided in good_name. We can summarize the average consumption and price levels for each good as:
