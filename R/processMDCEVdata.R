							 ...
							 model_options)
{
	...
	if (model_options$model_specification == "les"){
		model_type <- 1
		NAlpha <- 1
		NGamma <- J
	} else if (model_options$model_specification == "alpha"){
		model_type <- 2
		NAlpha <- J + 1
		NGamma <- 0
	} else if (model_options$model_specification == "gamma"){
		model_type <- 3
		NAlpha <- 1
		NGamma <- J
	} else if (model_options$model_specification == "gamma0"){
		model_type <- 4
		NAlpha <- 0
		NGamma <- J
	} else
	...
	if (model_options$n_classes > 1){
		...
		stan_dat$K <- model_options$n_classes
	}
return(stan_dat)
}
