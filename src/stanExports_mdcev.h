...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(119, 116, "end", "/common/mdcev_ll.stan");
    reader.add_event(119, 4, "restart", "model_mdcev");
    reader.add_event(125, 10, "include", "/common/mdcev_data.stan");
    reader.add_event(125, 0, "start", "/common/mdcev_data.stan");
    reader.add_event(148, 23, "end", "/common/mdcev_data.stan");
    reader.add_event(148, 11, "restart", "model_mdcev");
    reader.add_event(157, 20, "include", "/common/mdcev_tdata.stan");
    reader.add_event(157, 0, "start", "/common/mdcev_tdata.stan");
    reader.add_event(197, 40, "end", "/common/mdcev_tdata.stan");
    reader.add_event(197, 21, "restart", "model_mdcev");
    reader.add_event(312, 134, "end", "model_mdcev");
    return reader;
}
...
mdcev_ll(const Eigen::Matrix<T0__, Eigen::Dynamic, Eigen::Dynamic>& quant_j,
    ...
    try {
        ...
        validate_non_negative_index("f", "J", J);
        Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, Eigen::Dynamic> f(I, J);
        stan::math::initialize(f, DUMMY_VAR__);
        stan::math::fill(f, DUMMY_VAR__);
        stan::math::assign(f,elt_divide(subtract(1, alpha_j), add(quant_j, gamma_j)));
        ...
        stan::math::assign(log_like, add(subtract(add(add(add(add(multiply(subtract(1, M), stan::math::log(scale_full)), logf1), v1), rows_dot_product(nonzero, add(stan::math::log(f), v_j))), stan::math::log(add(inv(stan::math::exp(logf1)), rows_dot_product(nonzero, elt_divide(price_j, f))))), elt_multiply(M, stan::math::log(add(stan::math::exp(v1), multiply(stan::math::exp(v_j), ones_j))))), log_M_fact));
...
kt_ll(const Eigen::Matrix<T0__, Eigen::Dynamic, Eigen::Dynamic>& quant_j,
    ...
    try {
        ...
        if (as_bool(logical_eq(jacobian_analytical_grad, 0))) {
            ...
            stan::math::assign(log_j_det, add(add(subtract(stan::math::log(subtract(1, alpha)), log_num), rows_dot_product(nonzero, stan::math::log(inv(phi_quant_term)))), stan::math::log(add(elt_divide(stan::math::exp(log_num), subtract(1, alpha)), rows_dot_product(nonzero, elt_multiply(phi_quant_term, price_j))))));
        }
        ...
        if (as_bool(logical_eq(trunc_data, 1))) {
            ...
            stan::math::assign(log_like, subtract(add(log_j_det, log_like), stan::math::log(subtract(1, like_trunc))));
            }
        } else {
