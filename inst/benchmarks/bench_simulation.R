...
results_sim <- bench::press(
  nerrs = c(10, 50, 100, 500),
  sim_type = c("welfare", "demand"),
  {
    bench::mark(
      mdcev.sim(df_sim$df_indiv,
                df_common = df_sim$df_common,
                sim_options = df_sim$sim_options,
                cond_err = 1, nerrs = nerrs, sim_type = sim_type),
...
results_nsims <- bench::press(
  nsims = c(1, 5, 20, 50),
  {
    ...
    bench::mark(
      mdcev.sim(df_s$df_indiv,
                df_common = df_s$df_common,
                sim_options = df_s$sim_options,
                cond_err = 1, nerrs = 10, sim_type = "welfare"),
      iterations = 5, check = FALSE
    )
  }
)
  ...
  cond_err = c(0L, 1L),
  {
    bench::mark(
      mdcev.sim(df_sim$df_indiv,
                df_common = df_sim$df_common,
                sim_options = df_sim$sim_options,
                cond_err = cond_err, nerrs = 50, sim_type = "welfare"),
      iterations = 5, check = FALSE
    )
  }
)
